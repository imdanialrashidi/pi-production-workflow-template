...
3. Make the smallest direct patch. Preserve surrounding behavior and user changes; skip unrelated cleanup, abstractions, dependency changes, and documentation churn.
...
5. Inspect the scoped diff and run `git diff --check`. Stop when the target is proven; do not add reviewer or security ceremony to a qualifying Localized change.
...
Keep the response compact: result, changed file(s), exact check and observed outcome, and Git state/action. Do not emit a large criterion matrix for a qualifying quick fix. Never stage, commit, push, publish, deploy, or mutate an external system without the exact authorization required by `docs/GIT_POLICY.md`.
