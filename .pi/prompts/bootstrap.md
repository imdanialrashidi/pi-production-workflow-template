...
Do not expose secrets, deploy, install speculative infrastructure, or modify Pi harness-policy files (`AGENTS.md`, `docs/HARNESS.md`, `.pi/**`, `.mcp.json`, launcher/safety/doctor files) unless the user explicitly requested harness maintenance. For user-requested edits, prepare the existing `ai-changes` branch before editing and finish with scoped automatic PR delivery under `docs/GIT_POLICY.md`; local-only/read-only work never publishes. Other Git/GitHub actions still need exact owner authorization.
